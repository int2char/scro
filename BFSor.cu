...
__global__ void BFSFu(int *rudu,int*rudw,int *rid,int *d,int*p,int K,int EE,int PN,int off,int leveloff,int yel,int ye,int*mm,int *ss)
{
	int i = threadIdx.x + blockIdx.x*blockDim.x;
	if(i>=yel)return;
	int ly=i/ye+leveloff;
	int nid=blockIdx.y;
	int ii=nid*yel+i+off;
	if(d[ii]!=K)return;
	int dm=d[ii];
	for(int k=mm[nid];k<ss[nid]+mm[nid];k++)
		{
			...
			int v=d[off+node+i];
			if(v>dm+1){d[off+node+i]=K+1;}
		}
}
__global__ void BFScolor(int *rudu,int*rudw,int *rid,int *d,int*p,int K,int EE,int PN,int off,int leveloff,int yel,int ye,int*mm,int *ss)
{
	int i = threadIdx.x + blockIdx.x*blockDim.x;
	if(i>=yel)return;
	int ly=i/ye+leveloff;
	...
	for(int k=mm[nid];k<ss[nid]+mm[nid];k++)
		{
			...
			if(rudw[k+EE*ly]<0)continue;
			int v=d[off+node+i];
			if(v==d[ii]+1){p[off+node+i]=rid[k];}
		}
}
