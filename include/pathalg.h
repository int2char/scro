					...
					for(int k=L[y-1];k<L[y];k++)
					{
						...
						{	
							...
							for(int kk=0;kk<WD;kk++)
							{
									...
									{
										if(d[neioff+i]<INT_MAX/2)
											for(int m=0;m<nein[k][i].size();m++)
											{
												if(esigns[k][neieid[k][i][m]]>0)
												{
													if(neie[k][i][m]+d[neioff+i]<d[neioff+pnodesize+nein[k][i][m]])
														{
														 	 d[neioff+pnodesize+nein[k][i][m]]=d[neioff+i]+neie[k][i][m];
														 	 p[neioff+pnodesize+nein[k][i][m]+off]=neieid[k][i][m];
														}
												}
											}
									}
